...
<adt> ::= `type' <genericIdent>`\{'[<cosntructorLists>]`\}'<terminator><exp>

<members> ::= <ident>`:'<type>[`,'<ident>`:'<type>]*

<record> ::= `record' <genericIdent>`\{'[<members>]`\}'<terminator><exp>

<signatures> ::= <ident>`='<type>[`,'<ident>`='<type>]*
 
<typeclass> ::= `typeclass' <genericIdent>[`=>' <ident>]`\{'[<signatures>]`\}'<terminator><exp>

<instance> ::= `instance' <ident>`:'<ident>`\{'[<funDef>]*`\}'<terminator><exp>
